...
class FSFileUploadButton extends StatefulWidget {
  /// The storage instance to use.
  /// If not specified, [FirebaseStorage.instance] is used.
  final FirebaseStorage? storage;

  final  FSFolderNode uploadFolderNode;
  ...
  const FSFileUploadButton({
    ...
    required this.uploadFolderNode,
  });

  @override
  State<FSFileUploadButton> createState() => _FSFileUploadButtonState();
}

class _FSFileUploadButtonState extends State<FSFileUploadButton> {
  bool isLoading = false;

  FirebaseStorage get storage => widget.storage ?? FirebaseStorage.instance;

  late final filePicker = FilePicker(extensions: widget.extensions, mimeTypes: widget.mimeTypes);
  ...
  Future<void> _upload(FirebaseUIStorageConfiguration config) async {
    try {
      ...
      final ref = widget.uploadFolderNode.ref.child(childRef);
...
// copied from config.dart, because wasn't exported
extension FirebaseUIStorageContextExtensions on BuildContext {
  FirebaseUIStorageConfiguration configFor(FirebaseStorage storage) {
    final w = dependOnInheritedWidgetOfExactType<FirebaseUIStorageConfigOverride>();
...
// copied from file_picker.dart, because wasn't exported
class FilePicker {
  final List<String> extensions;
  final List<String> mimeTypes;
  final XTypeGroup _typeGroup;

  FilePicker({this.extensions = const [], this.mimeTypes = const []}) : _typeGroup = XTypeGroup(extensions: extensions, mimeTypes: mimeTypes);
