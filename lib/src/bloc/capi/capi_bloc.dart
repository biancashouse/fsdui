...
class CAPIBloC extends HydratedBloc<CAPIEvent, CAPIState> {
  ...
  Future<void> _revertSnippet(
    ...
    SNode? revertedRootNode = await SNode.loadSnippetFromCacheOrFromFB(
      snippetName: event.snippetName,
    );
  ...
  // Future<void> _pageAdded(PageAdded event, Emitter<CAPIState> emit) async {
  //   emit(state.copyWith(force: (state.force??0) + 1));
  // }

  // Future<void> _deletePage(DeletePage event, Emitter<CAPIState> emit) async {
  //   emit(state.copyWith(force: (state.force??0) + 1));
  // }
  ...
  void _forceRefresh(ForceRefresh event, Emitter<CAPIState> emit) {
    fsdui.logger.i(
      "forceRefresh --------------------------------------------------------",
    );
    emit(state.copyWith(force: (state.force ?? 0) + 1));
    ...
    if (event.onlyTargetsWrappers) {
      emit(
        state.copyWith(force: (state.force ?? 0), onlyTargetsWrappers: false),
      );
    }
  }
  ...
  Future<void> _replaceSnippetFromJson(
    ...
    if (event.snippetJson == null) {
      ...
      rootNode = SNodeMapper.fromJson(snippetJson);
      rootNode.name = event.snippetBeingReplaced;
    } else {
      rootNode = SNodeMapper.fromJson(event.snippetJson!);
      rootNode.name = event.snippetBeingReplaced;
    }
    fsdui.logger.i(
      '_replaceSnippetFromJson: snippet name is "${rootNode.name}"',
    );
  ...
  // void _selectPanel(SelectPanel event, Emitter<CAPIState> emit) {
  //   // null clears selection
  //   emit(
  //     state.copyWith(selectedPanel: event.panelName, force: (state.force??0) + 1),
  //   );
  // }
  ...
  // void _setPanelOrPlaceholderSnippet(SetPanelSnippet event, Emitter<CAPIState> emit) {
  ...
  void _selectNode(SelectNode event, Emitter<CAPIState> emit) {
  ...
  void _clearNodeSelection(ClearNodeSelection event, Emitter<CAPIState> emit) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
  ...
  Future<void> _completeDeletion(
    ...
    SnippetTreeController possiblyNewTreeC = state.snippetBeingEdited!.treeC;
    // if (newSel.getParent() .isASnippetRoot) {
    //   possiblyNewTreeC = SnippetTreeController(
    //     roots: [newSel],
    //     childrenProvider: Node.snippetTreeChildrenProvider,
    //     parentProvider: Node.snippetTreeParentProvider,
  ...
  // Future<void> _showCutout(ShowCutout event, Emitter<CAPIState> emit) async {
  //   if (state.snippetBeingEdited?.aNodeIsNotSelected??false) return;
  //
  //   Rect? r =
  ...
  SNode _possiblyRemoveFromParent() {
    ...
    if (selParent.isASnippetRoot && sel.hasNoChildren()) {
    ...
    //  if (sel.hasNoChildren() || sel .isASnippetRoot) {
    //    newSel = sel.removeFromParent();
    //    return newSel;
    //  }
    //
    // if (sel .isASnippetRoot) {
    //    newSel = sel.removeFromParent();
    ...
    //      ..setParent(selParent);
    //    newSel = selParent;
    //  } else if (selParent is MC && (sel is CL || sel .isASnippetRoot)) {
    //    selParent.children.remove(sel);
    //    newSel = selParent;
    //  } else
    //  if (selParent is ListViewNode && (sel is CL || sel .isASnippetRoot)) {
    //    selParent.children.remove(sel);
    //    newSel = selParent;
    //  } else
    //  if (selParent is GridViewNode && (sel is CL || sel .isASnippetRoot)) {
    //    selParent.children.remove(sel);
    //    newSel = selParent;
    //  } else if (selParent is CustomScrollViewNode &&
    //      (sel is CL || sel .isASnippetRoot)) {
  ...
  Future<void> _cutNode(CutNode event, Emitter<CAPIState> emit) async {
    _pushUndo();
  ...
  Future<void> _copyNode(CopyNode event, Emitter<CAPIState> emit) async {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _updateClipboard(event.node);
    emit(state.copyWith(force: (state.force ?? 0) + 1));
    fsdui.modelRepo.saveAppInfo();
  ...
  void _wrapWith(WrapSelectionWith event, Emitter<CAPIState> emit) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _pushUndo();
    ...
    if (w is ScaffoldNode) {
      ...
      if ((w is! SC && w is! MC && w is! ListViewNode && w is! GridViewNode) ||
          w is WidgetSpanNode) {
        return;
      }
    ...
    if (selectedNodeParent == null) {
      ...
      SnippetInfoModel? snippetInfo = fsdui.appInfo.cachedSnippetInfo(w.name!);
      if (snippetInfo != null) {
        ...
        if (currVersionId != null) {
          ...
          snippetInfo.cacheVersion(currVersionId, w);
        }
      }
    }
  ...
  void _replaceWith(ReplaceSelectionWith event, Emitter<CAPIState> emit) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _pushUndo();
  ...
  void _replaceWithNewNodeOrClipboard(SNode sel, emit, SNode r) {
    ...
    if (parent == null) {
      // root was replaced - new treeC needed
      possiblyNewTreeC = SnippetTreeController(
        roots: [r],
        childrenProvider: SNode.childrenProvider,
        parentProvider: SNode.parentProvider,
    ...
    emit(
      state.copyWith(
        force: (state.force ?? 0) + 1,
        snippetBeingEdited: newSnippetBeingEdited,
      ),
    );
  }

  void _addChild(AppendChild event, Emitter<CAPIState> emit) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _pushUndo();
  ...
  void _prependArticle(PrependArticle event, Emitter<CAPIState> emit) {
    _pushUndo();
  ...
  void _pasteReplacement(PasteReplacement event, Emitter<CAPIState> emit) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _pushUndo();
    if (fsdui.appInfo.clipboard != null) {
      ...
      _replaceWithNewNodeOrClipboard(
        selectedNode,
        emit,
        fsdui.appInfo.clipboard!.clone(),
      );
    }
  }

  void _pasteChild(PasteChild event, Emitter<CAPIState> emit) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _pushUndo();
    if (fsdui.appInfo.clipboard != null) {
      SNode selectedNode = state.snippetBeingEdited!.selectedNode!;
      _addOrPasteChild(selectedNode, emit, fsdui.appInfo.clipboard!.clone());
    }
  }

  void _addSibling(event, Emitter<CAPIState> emit, {required bool before}) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _pushUndo();
    SNode selectedNode = state.snippetBeingEdited!.selectedNode!;
  ...
  void _pasteSiblingBefore(PasteSiblingBefore event, Emitter<CAPIState> emit) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _pushUndo();
    SNode selectedNode = state.snippetBeingEdited!.selectedNode!;
    if (fsdui.appInfo.clipboard != null) {
  ...
  void _pasteSiblingAfter(PasteSiblingAfter event, Emitter<CAPIState> emit) {
    if (state.snippetBeingEdited?.aNodeIsNotSelected ?? false) return;
    _pushUndo();
    SNode selectedNode = state.snippetBeingEdited!.selectedNode!;
    SNode clipboardNode = fsdui.appInfo.clipboard!;
    if (state.snippetBeingEdited!.selectedNode?.getParent() is MC) {
  ...
  void _reorderSibling(ReorderSibling event, Emitter<CAPIState> emit) {
  ...
  void _addSiblingAt(SNode newNode, Emitter<CAPIState> emit, int i) {
    // state.snippetBeingEdited?.newVersion();
    SNode rootNode = state.snippetBeingEdited!.getRootNode();
  ...
  void _pasteSiblingAt(SNode clipboardNode, Emitter<CAPIState> emit, int i) {
    // state.snippetBeingEdited?.newVersion();
  ...
  void _applyUndoRedoJson(String json, Emitter<CAPIState> emit) {
    final restored = SNodeMapper.fromJson(json);
    restored
        .validateTree(); // re-establishes parent references lost during JSON serialization
    final snippetInfo = fsdui.appInfo.cachedSnippetInfo(restored.name!);
  ...
  void _undo(Undo event, Emitter<CAPIState> emit) {
    final sbe = state.snippetBeingEdited;
  ...
  void _redo(Redo event, Emitter<CAPIState> emit) {
    final sbe = state.snippetBeingEdited;
