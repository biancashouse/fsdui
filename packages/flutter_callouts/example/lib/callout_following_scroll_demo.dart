...
class _ScrollingDemoState extends State<ScrollingDemo> {
  ...
  late CalloutConfig _greenCC, _yellowCC, _orangeCC;
  ...
  void initState() {
    ...
    _greenCC = _createGreenCalloutConfig();
    _yellowCC = _createYellowCalloutConfig();
                            ...
                            Timer(Duration(seconds: 5), () {
                              if (!didScroll) {
                                fca.showToast(
                                  ...
                                  msg:
                                      'scroll to see the callout pointers follow their target',
  ...
  double get fontSize {
    double result = fca.scrW < 1367 ? 12.0 : 24.0;
    return result;
  }
  ...
  CalloutConfig _createYellowCalloutConfig() => CalloutConfig(
    ...
    // elevation: 10,
    targetPointerType: TargetPointerType.wavy_line(),
    animatePointer: true,
    followScroll: false,
    // resizeableH: true,
    // resizeableV: true,
  ...
  Widget _createBlueContent() => IntrinsicHeight(
    child: Container(
      ...
      child: fca.coloredText(
        ...
        'have their size (initialCalloutW and\n'
        'initialCallout) in the source code.',
        color: Colors.white,
      ),
    ),
  );
  ...
  void toggleFollowScroll1() {
    ...
    _greenCC.followScroll = followScroll1;
    fca.rebuild(_greenCC.cId);
    // _yellowCC.movedOrResizedNotifier?.value++;
    // _orangeCC.movedOrResizedNotifier?.value++;
  ...
  void toggleFollowScroll2() {
    ...
    _yellowCC.followScroll = followScroll2;
    _orangeCC.followScroll = followScroll2;
    // _greenCC.movedOrResizedNotifier?.value++;
    // _yellowCC.movedOrResizedNotifier?.value++;
    // _orangeCC.movedOrResizedNotifier?.value++;
    // fca.rebuild('some-callout-id-1');
    // fca.rebuild('some-callout-id-2');
    ...
    // fca.refreshAll();
    // });
    // force a refresh by scrolling by 1 px
    _yellowCC.movedOrResizedNotifier?.value++;
    _orangeCC.movedOrResizedNotifier?.value++;
  }
